...
dotnet add tests/Generator.Web.Tests package bunit
```
...
public class StartPageTests : TestContext
{
...
public class ProposalsPageTests : TestContext
{
    ...
    public async Task Pokazuje_trzy_propozycje_jako_podglady_w_iframe()
    {
        ...
        var cut = RenderComponent<Proposals>(ps => ps.Add(p => p.ProjectId, id));
    ...
    public async Task Podczas_generowania_widac_ze_cos_sie_dzieje()
    {
        ...
        var cut = RenderComponent<Proposals>(ps => ps.Add(x => x.ProjectId, p.Id));
...
- Produces:
  ...
  - `record ClickPayload(string? Anchor, double X, double Y, string Viewport)`
  - **Kontrakt skryptu w iframe** (implementacja Twoja): na klik wysyła `window.parent.postMessage({type:'cmt-click', anchor, x, y, viewport}, '*')`, gdzie `anchor` to wartość `data-cmt-id` najbliższego przodka albo `null`.
...
public class CommentBridgeTests : TestContext
{
    [Fact]
    public async Task Klik_w_blok_tworzy_komentarz_z_anchorem_i_guidem_z_przegladarki()
    {
        ...
        var cut = RenderComponent<CommentBridge>(ps => ps
...
@code {
    ...
    public record ClickPayload(string? Anchor, double X, double Y, string Viewport);
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        await JS.InvokeVoidAsync("commentBridge.listen", _self);
    }
...
`src/Generator.Web/wwwroot/js/preview-click.js` — skrypt wstrzykiwany do iframe. **Implementacja Twoja**, kontrakt: na klik wyślij `{type:'cmt-click', anchor, x, y, viewport}` do `window.parent`, gdzie `anchor` to `data-cmt-id` najbliższego przodka albo `null`:
...
public class CommentListTests : TestContext
{
    ...
    public void Note_jest_przy_komentarzu_nie_w_zbiorczym_podsumowaniu()
    {
        ...
        var cut = RenderComponent<CommentList>(ps => ps
...
public class JobStatusTests : TestContext
{
    [Fact]
    public void Running_pokazuje_ze_pracujemy_bez_szczegolow_technicznych()
    {
        var cut = RenderComponent<JobStatus>(ps => ps
    ...
    {
        var cut = RenderComponent<JobStatus>(ps => ps.Add(p => p.Job, (JobView?)null));
...
public class EditorPageTests : TestContext
{
    ...
    public async Task Klient_nie_ma_zadnego_pola_do_edycji_strony()
    {
        ...
        var (_, id) = await Setup();
        var cut = RenderComponent<Editor>(ps => ps.Add(p => p.ProjectId, id));
    ...
    public async Task Osierocone_kotwice_sa_widoczne_nigdy_wyciszane()
    {
        ...
        var cut = RenderComponent<Editor>(ps => ps.Add(x => x.ProjectId, id));
...
- [ ] **Step 2: Dodaj `ForceOrphaned` do mocka i uruchom testy**
